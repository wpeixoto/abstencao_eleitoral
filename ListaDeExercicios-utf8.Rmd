...
e. Considerando o tempo que a pessoa permaneceu empregada (**tenure**), qual dos estados relaciondados (21, 44, 58, 82 e 94) proporciona maior establilidade no emprego? <span style="color:red">Pendente</span>
...
osestados = desempregados[desempregados$state %in% c(21, 44, 58, 82, 94),] # Pra quê essa vírgula?
# 1. 
"Estado x Sexo"
table(osestados$state, osestados$sex)

# 2. Estado x Anos de escolaridade (>12)
table(osestados$state, osestados$school12)

# 3.Motivo da perda do emprego
#empregos_perdidos = table(osestados$joblost, )
...
```{r}
# qtd municípios
length(table(votZona$CODIGO_MUNICIPIO))

# qtd zonas eleitorais
length(table(votZona$NUMERO_ZONA))
